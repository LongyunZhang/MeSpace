...
public class DBManager {
    ...
    public List<ItemInfo> queryLikeItemname(String itemname) {
        Log.d("sql", itemname);
        ArrayList<ItemInfo> items = new ArrayList<ItemInfo>();
        Log.d("sql","SELECT * FROM item_table where itemName like");
        Cursor c = db.rawQuery("SELECT * FROM item_table where itemName like ?", new String[]{itemname});
        while (c.moveToNext()) {
            ItemInfo item = new ItemInfo();
            item.setId(c.getInt(c.getColumnIndex("id")));
            item.setItemName(c.getString(c.getColumnIndex("itemName")));
